...
from .mrdalane_head import MRDALaneHead
...
class MRDALane(nn.Module):
    def __init__(self, args):
        ...
        _dim_ = args.mrdalane_cfg.fpn_dim
        num_query = args.mrdalane_cfg.num_query
        num_group = args.mrdalane_cfg.num_group
        sparse_num_group = args.mrdalane_cfg.sparse_num_group

        self.encoder = build_backbone(args.mrdalane_cfg.encoder)
        if getattr(args.mrdalane_cfg, 'neck', None):
            self.neck = build_neck(args.mrdalane_cfg.neck)
        else:
            self.neck = None
        ...
        self.head = MRDALaneHead(
            ...
            **args.mrdalane_cfg.get('head', {}),
            trans_params=args.mrdalane_cfg.get('trans_params', {})
        )
