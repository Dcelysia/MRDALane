...
import traceback
import shutil
from utils.Camera_corruptions import ImageAddSnow, ImageAddFog, ImageAddRain, ImageAddGaussianNoise
from utils.Camera_corruptions import *
from data.Load_Data import *
from data.apollo_dataset import ApolloLaneDataset
from models.mrdalane import MRDALane
...
class Runner:
    ...
    def validate(self, model, **kwargs):
        ...
        with torch.no_grad():
            ...
            # 初始化模型
            # snow_sim = ImageMotionBlurFrontBack(severity=3)
            # snow_sim = ImageAddFog(severity=3, seed=2022)
            for i, extra_dict in enumerate(loader):
                ...
                if not args.no_cuda:
                    ...
                    image = extra_dict['image']
                #  进行鲁棒性测试
                # for i, single in enumerate(image):
                #     img_bgr_255_np_float32 = single.permute(1, 2, 0).cpu().numpy()  
                #     img_bgr_255_np_uint8 = (img_bgr_255_np_float32 * 255).astype(np.uint8)
                #     img_rgb_255_np_uint8 = img_bgr_255_np_uint8[:, :, [2, 1, 0]]
                #     image_aug_rgb = snow_sim(image=img_rgb_255_np_uint8)
                #     image_aug_bgr = image_aug_rgb[:, :, [2, 1, 0]]
                #     image_aug_bgr_float32 = image_aug_bgr.astype(np.float32) / 255.0
                #     image_aug_bgr_tensor = torch.from_numpy(image_aug_bgr_float32).permute(2, 0, 1).to(single.device)
                #     image[i] = image_aug_bgr_tensor
                ...
                for j in range(num_el):
                    ...
                    json_file = json_file[55:] #rare 58 #standard 55 illu 56
